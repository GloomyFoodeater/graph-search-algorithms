...
var
  NewGraph: TGraph;
  // NewGraph - Копия графа для открытия
...
begin
  ...
  try
    ...
    MessageBox(fmEditor.Handle, 'Ошибка при открытии графа', 'Ошибка', MB_OK);
  end;
end;
...
begin
  ...
  begin
    ...
    // Рисование на холсте сохраняемого изображения
    try
...
begin
  ...
  begin
    try
      ...
      begin
        DestroyGraph(NewGraph);
        MessageBox(Handle, PWideChar(E.Message), 'Ошибка', MB_OK);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    MessageBox(fmEditor.Handle, 'Путь не найден.', 'Информация о пути', MB_OK);
end;
