...
{
  Модуль с типами и подпрограммами
  для работы с динамическими структурами,
  необходимыми для алгоритмов поиска.
}
...
type
  ...
  // Тип односвязного списка с натуральными числами
  TPItem = ^TItem;
  ...
  // Тип стека с натуральными числами
  TStack = TPItem;

  // Тип очереди с натуральными числами
  TQueue = record
    Head: TPItem;
    Tail: TPItem;
  end;
...
var
  Item: TPItem;
  // Item - Ссылка на освобождаемый узел
...
var
  Item: TPItem;
  // Item - Ссылка на вставляемое звено
...
var
  Item: TPItem;
  // Item - Ссылка на извлекаемое звено
...
begin
  ...
  begin
    ...
    // Извлечение элемента с освобождением указателя
    Result := Item.Number;
    Dispose(Item);
  end
  else
